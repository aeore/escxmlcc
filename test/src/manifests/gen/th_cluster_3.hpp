...
public:
	...
	{
	public:
		virtual state* trigger_w( CTh_cluster_3FSM& ) { return 0; }
		virtual state* unconditional( CTh_cluster_3FSM& ) { return 0; }
		virtual state* unconditional_async( CTh_cluster_3FSM& ) { return 0; }
...
private: 
	...
	public:
		// --------------------------------------------------------------------------
		CPAsyncEventTransitionData( CTh_cluster_3FSM& fsm, const event trigger ): mFSM( fsm ), mTrigger( trigger ) { /* none */ }
		// --------------------------------------------------------------------------
	...
	public:
		// --------------------------------------------------------------------------
		CPAsyncTriggerInvoker( CTh_cluster_3FSM& fsm ) : mFsm( fsm ) { /* none */ }
		// --------------------------------------------------------------------------
...
public:
	...
	// --------------------------------------------------------------------------
	CTh_cluster_3FSM( ITh_cluster_3ActionHandler* pActionHandler ) : cur_state( &m_scxml ), asyncTriggerInvoker( *this )
	// --------------------------------------------------------------------------
	{
	...
	{
		// --------------------------------------------------------------------------
		state* initial( CTh_cluster_3FSM&sc ) {
		// --------------------------------------------------------------------------
	...
	{
		// --------------------------------------------------------------------------
		inline virtual state* unconditional( CTh_cluster_3FSM &sc ) {
		// --------------------------------------------------------------------------
			int& a = sc.model.a;
	...
	{
		// --------------------------------------------------------------------------
		inline state* trigger_w( CTh_cluster_3FSM &sc ) {
		// --------------------------------------------------------------------------
			int& a = sc.model.a;
			if ( a > 2 ) return transition<6, &state::trigger_w, state_C, state_D>()( this, sc.m_state_D, sc );
			else return 0;
		}
...
// --------------------------------------------------------------------------
template<> inline void CTh_cluster_3FSM::transition_actions<6, &CTh_cluster_3FSM::state::trigger_w, CTh_cluster_3FSM::state_C, CTh_cluster_3FSM::state_D>::enter(CTh_cluster_3FSM::data_model& m )
// --------------------------------------------------------------------------
{
	m.actionHandler->onGap( m );
}
