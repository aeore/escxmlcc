...
public:
	...
	{
	public:
		virtual state* event_q( CInclude_1FSM& ) { return 0; }
...
public:
	...
	// --------------------------------------------------------------------------
	CInclude_1FSM( IInclude_1ActionHandler* pActionHandler ) : cur_state( &m_scxml )
	// --------------------------------------------------------------------------
	{
	...
	{
		state* initial( CInclude_1FSM&sc ) { return transition<0, &state::initial, scxml, state_init, internal>()( this, sc.m_state_init, sc ); }
	} m_scxml;

	class state_init: public composite<state_init, scxml>
	{
		state* event_q( CInclude_1FSM &sc ) {
			std::vector<int>& v = sc.model.v;
			if ( v.size() > 0 ) return transition<1, &state::event_q, state_init, state_full>()( this, sc.m_state_full, sc );
			if ( v.size() == 0 ) return transition<2, &state::event_q, state_init, state_empty>()( this, sc.m_state_empty, sc );
			else return 0;
		}
